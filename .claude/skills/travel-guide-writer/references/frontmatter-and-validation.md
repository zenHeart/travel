...
- 总览的 `## 出发准备` 下用有序类目嵌套 checkbox，不用 h3/h4 分类；避免目录变成准备用品目录。类目数不固定。
...
`validate-guide.mjs` 只检查显式指定的行程或单文件，覆盖新格式、真实日历日期、编号时间轴、总览清单、相对文档与锚点、本地图片；不强制历史游记重写，不把未定日期变成编造的日期。链接锚点复用 `src/utils/toc.ts`，本网站只有 h2/h3 进入目录并获得该锚点，不能假定 h4 店名可跳转。
