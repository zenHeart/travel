...
export class ContentScanner {
  ...
  private async scanCityDirectory(
    ...
    try {
      ...
      console.log(
        `扫描${status}城市，找到${Object.keys(contentModules).length}个文件:`
      );
      ...
      for (const [path, content] of Object.entries(contentModules)) {
        ...
        console.log(
          `处理文件: ${path}, 状态: ${fileStatus}, 城市ID: ${cityId}`
        );

        if (cityId && content && fileStatus === status) {
          const city = await this.buildCityData(
            cityId,
            status,
            content as string
          );
  ...
  private async buildCityData(
    ...
    try {
      ...
      const visitDate =
        frontmatter.visit_date || this.extractVisitDate(indexContent);
      const duration =
        frontmatter.duration || this.extractDuration(indexContent);
